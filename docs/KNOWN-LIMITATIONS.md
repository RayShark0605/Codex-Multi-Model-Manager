...
- 自动持久化目前只对本机 loopback LM Studio `0.4.21.x` 已确认的 per-model defaults JSON 结构开放。未知版本、远程 endpoint、目标 defaults 文件缺失/过大/过深、根结构不符、路径包含 reparse point/junction、concrete identity 不唯一或 Prompt Template 为未知自定义内容时，自动 Switch 会在任何文件写入和 unload 前 fail closed；只读分析、导出和 LM Studio My Models 手工设置仍可用。
